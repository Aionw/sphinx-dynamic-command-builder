...
def _copy_static_assets(app, exc):
    ...
    copy_asset(str(STATIC_DIR / "sphinx-dynamic-command.css"), str(static_out))
    copy_asset(str(STATIC_DIR / "sphinx-dynamic-command.js"), str(static_out))


def setup(app):
    app.add_directive("dynamic-command", DynamicCommandDirective)
    app.add_css_file("sphinx-dynamic-command.css")
    app.add_js_file("sphinx-dynamic-command.js")
